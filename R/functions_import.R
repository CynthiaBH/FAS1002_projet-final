...
import_data <- function(filename, sheetname){
    data <- read_excel(paste0("data/raw/", filename), sheet=sheetname)

}

download_data("data/raw/", "covid_data", url_covid,".csv")
file_covid <- list.files("data/raw", pattern = "covid")
covid_data <- read.csv(paste0("data/raw/", file_covid))

download_data("data/raw/", "pib_data", url_pib,".xlsx")
file_pib <- list.files("data/raw", pattern = "pib")
pib_data <-import_data(file_pib, "data-for-countries-etc-by-year")

download_data("data/raw/", "lifeexp_date", url_life_ex, ".xlsx")
